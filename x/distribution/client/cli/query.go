...
func (q ValidatorQuerierCli) GetValidatorCommission(height int64, validatorAddress sdk.ValAddress) (sdk.DecCoins, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "Unable to query validator commission")
	}
...
func (q ValidatorQuerierCli) GetValidatorRewards(height int64, validatorAddress sdk.ValAddress) (sdk.DecCoins, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "Unable to query validator reward")
	}
...
func findBlockHeightFromDate(startTime time.Time, node rpcclient.Client, maxHeight int64) (*int64, error) {
	...
	minBlock, err := node.Block(&minHeight)
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("Unable to get block %d from node", minHeight))
	}

	if minBlock.BlockMeta.Header.Time.Unix() >= searchUnix {
	...
	for minHeight <= maxHeight {
		...
		if err != nil {
			return nil, errors.Wrap(err, fmt.Sprintf("Unable to get block %d from node", currentHeight))
		}
		previousBlock, err := node.Block(&previousHeight)
		if err != nil {
			return nil, errors.Wrap(err, fmt.Sprintf("Unable to get block %d from node", previousHeight))
		}
...
func calculateValidatorRewardsAndCommissionPerDay(
	...
	for currentHeight <= lastBlockHeight {
		currentBlock, err := node.Block(&currentHeight)
		if err != nil {
			err = errors.Wrap(err, "Unable to get block")
			return
		}
		...
		if err != nil {
			err = errors.Wrap(err, "Unable to get validator commission")
			return
		}
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			startTime, err := time.Parse(DATE_FORMAT, args[0])
			if err != nil {
			...
			if err != nil {
				return errors.Wrap(err, "Unable to find block height from start time")
			}

			endTime, err := time.Parse(DATE_FORMAT, args[1])
			if err != nil {
			...
			for _, bech32 := range args[2:] {
				...
				if err != nil {
					return errors.Wrap(err, "Unable to parse validator address: "+bech32)
				}
